...
@gallery_routes.route('/gallery')
def gallery():
uploads_dir = current_app.config['UPLOAD_FOLDER']
if not os.path.exists(uploads_dir):
return jsonify({"images": []})
files = [f for f in os.listdir(uploads_dir) if os.path.isfile(os.path.join(uploads_dir, f))]
# Return full public URLs for each file
base = (current_app.config.get('BASE_URL') or '')
urls = [f"{base}/uploads/images/{f}" for f in files]
return jsonify({"images": urls})




@gallery_routes.route('/uploads/images/<path:filename>')
def uploaded_file(filename):
uploads_dir = current_app.config['UPLOAD_FOLDER']
file_path = os.path.join(uploads_dir, filename)
if not os.path.exists(file_path):
abort(404)
# send_from_directory will stream the file
